...
export default function ModalRegister() {
  ...
  const cbAgree = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const computeTotalPayment = () => {
    ...
    if (buyBottle) extraItems += 200;
    if (buyShoes) extraItems += 600;
    if (buyCap) extraItems += 400;

    total =
      (buyBottle && buyShoes && buyCap ? (80 / 100) * extraItems : extraItems) +
      total;
